...
// BAGON REMOVED : EVT_MENU(BG_SEGM_SAVE_EDGEMAP, BgMdiSegmentChild::OnSaveBoundaries)
...
void BgMdiFrame::ZoomControl(wxCommandEvent& event)
{
	...
	switch (event.GetId())/* BAGON (event.m_id) */
	{
	case BG_CROSS:
	...
	if(activeChild->GetId()	== BG_EDGE_WINDOW)
	{
		switch (event.GetId()) /* BAGON CHANGED (event.m_id) */
		{
		case BG_CROSS:
...
/* BAGON CHANHGED     pbitmap = new wxBitmap(pimage); */
...
void BgImCanvas::OnDraw(wxDC& dc)
{
	...
	if((zoom_window)&&(leaving))
	{
		...
		{
#if wxCHECK_VERSION(2, 3, 0)
/* BAGON CHANGED          dc.DrawBitmap(wxBitmap(refresh_box), cx, cy); */
...
int BgImCanvas::Zoom(int zconst, int m_x, int m_y)
{
	...
	if(hasbitmap)
	{
		...
		if(zoom_level == 1)
		{
			delete	pbitmap;
#if wxCHECK_VERSION(2, 3, 0)
/* BAGON CHANGED          pbitmap = new wxBitmap(pimage); */
			pbitmap = new wxBitmap(pimage->GetData());
#else
			pbitmap	= new wxBitmap(pimage->ConvertToBitmap());
#endif
			if(zImg)	delete zImg;
...
/* BAGON 		myevent.m_id = BG_CANVAS_VIEW1_CONFMAP; */
		myevent.SetId(BG_CANVAS_VIEW1_CONFMAP);
		OnUpdatePlotWindow1(myevent);
/* BAGON		myevent.m_id = BG_CANVAS_VIEW2_GRADMAP; */
